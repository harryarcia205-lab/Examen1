    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <form action="{{ route('factory_articles.update', $factory_article) }}" method="POST" id="form-edit-{{ $factory_article->id }}" onsubmit="confirmarActualizacion(event, {{ $factory_article->id }})" novalidate>
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        <label for="negotiation_cost" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Costo de Negociación</label>
                        <input type="number" id="negotiation_cost" name="negotiation_cost" value="{{ old('negotiation_cost', $factory_article->negotiation_cost) }}" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors" placeholder="Ej. 10.50">
                    ...
                    <div class="mb-6">
                        <label for="date_estimated" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Fecha Estimada</label>
                        <input type="date" id="date_estimated" name="date_estimated" value="{{ old('date_estimated', $factory_article->date_estimated) }}" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                        @error('date_estimated')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select id="article_id" name="article_id" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                            ...
                            @foreach($articles as $article)
                                <option value="{{ $article->id }}" {{ old('article_id', $factory_article->article_id) == $article->id ? 'selected' : '' }}>
                                    {{ $article->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-gray-100 dark:border-gray-700">
                        <a href="{{ route('factory_articles.index') }}" class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 disabled:opacity-25 transition ease-in-out duration-150">
                            Cancelar
                        </a>
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 shadow-lg shadow-indigo-500/30">
                            Actualizar Artículo de Fábrica
                        </button>
                    </div>
                </form>
    ...
    <script>
        function confirmarActualizacion(event, id) {
            ...
            Swal.fire({
                ...
                text: "Se actualizará la información de este artículo de fábrica.",
                icon: 'question',
