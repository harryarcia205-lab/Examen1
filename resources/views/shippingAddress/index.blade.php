    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse ($shippingAddress as $shippingAddress)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-mono text-gray-400 dark:text-gray-500">{{ $shippingAddress->id }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                                            {{ $shippingAddress->client->full_name ?? 'Cliente no encontrado' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1 max-w-xs truncate">
                                            {{ $shippingAddress->location_reference }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-600 dark:text-gray-400">
                                            {{ $shippingAddress->city }} <br>
                                            <span class="text-xs">{{ $shippingAddress->neighborhood }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end items-center space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <a href="{{ route('shippingAddress.show', $shippingAddress) }}" class="text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors" title="Ver">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                            </a>
                                            <a href="{{ route('shippingAddress.edit', $shippingAddress) }}" class="text-gray-400 hover:text-amber-500 transition-colors" title="Editar">
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmarEliminacion({{ $shippingAddress->id }})" class="text-gray-400 hover:text-red-500 transition-colors" title="Eliminar">
                ...
                @if(isset($shippingAddress) && method_exists($shippingAddress, 'hasPages') && $shippingAddress->hasPages())
