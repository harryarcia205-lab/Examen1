    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <!-- Article ID -->
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase mb-2">Article ID</h3>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $orderLine->article_id }}</p>
                </div>

                <!-- Requested Quantity -->
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase mb-2">Requested Quantity</h3>
                    <p class="text-gray-700 dark:text-gray-300">{{ $orderLine->requested_quantity }}</p>
                </div>

                <!-- Unit Price -->
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase mb-2">Unit Price</h3>
                    <p class="text-gray-700 dark:text-gray-300">${{ number_format($orderLine->unit_price, 2) }}</p>
                </div>

                <!-- Line Subtotal -->
                <div class="mb-6">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase mb-2">Line Subtotal</h3>
                    <p class="text-gray-700 dark:text-gray-300">${{ number_format($orderLine->line_subtotal, 2) }}</p>
                </div>

                <!-- Actions -->
                <div class="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-gray-100 dark:border-gray-700">
                    <form action="{{ route('order_lines.destroy', $orderLine) }}" method="POST" class="inline" id="form-delete-{{ $orderLine->id }}">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDelete({{ $orderLine->id }})" 
                    ...
                    <a href="{{ route('order_lines.edit', $orderLine) }}" 
